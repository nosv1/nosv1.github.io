...
function runStatistics() {
    ...
    for (let i = 0; i < sessionArray.length; i++)
        if (sessionArray[i] != 0) sessionCount++;

    if (sessionCount > 0) {
        statisticsTimeout = window.setTimeout(() => runSim(9999), 0);
    } else {
...
function runSim(count) {
    ...
    // calc median shower length
    if (statistics.showerLength.length > 0) {
        statistics.showerLength.sort((a, b) => a - b);
        let mid = Math.floor(statistics.showerLength.length / 2);
        if (statistics.showerLength.length % 2 === 0) {
            statistics.medianShowerLength = (statistics.showerLength[mid - 1] + statistics.showerLength[mid]) / 2;
        } else {
